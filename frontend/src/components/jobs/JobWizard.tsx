...
function buildSteps(backupSteps: BackupStepsConfig): unknown[] {
  ...
  if (backupSteps.appdata) {
    ...
    pushWithRetention({
      ...
      config: {
        targetId: a.targetId,
        allContainersDynamic: a.allContainersDynamic,
        containers: a.allContainersDynamic ? [] : a.containers,
        stopContainers: a.stopContainers,
        stopOrder: a.allContainersDynamic ? [] : (a.stopOrder.length > 0 ? a.stopOrder : a.containers),
        excludedContainers: a.allContainersDynamic ? a.excludedContainers : [],
        stopOrderPriority: a.allContainersDynamic ? a.stopOrderPriority : [],
      ...
      for (const s of jobSteps) {
        ...
        if (s.type === 'flash') {
          ...
          newBackupSteps.appdata = {
            ...
            targetId: (c.targetId as string) ?? '',
            allContainersDynamic: c.allContainersDynamic === true,
            containers: (c.containers as string[]) ?? [],
            excludedContainers: (c.excludedContainers as string[]) ?? [],
            stopOrderPriority: (c.stopOrderPriority as string[]) ?? [],
